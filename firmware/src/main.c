/*
 * RP2350 host firmware for tt_um_brainfck_asic (Tiny Tapeout demo board v3).
 *
 * Core 0 (this file): drives the ASIC clock, feeds instructions, mirrors
 * the program counter, resolves bracket jumps against a precomputed match
 ...
 * an instruction twice.  We generate the ASIC clock ourselves (PWM) and
...
/* ---- configuration ---- */
#define ASIC_CLK_HZ 1000000u /* keep <= ~2 MHz: serial RX/TX and the      \
                                instr_valid pulse are bit-banged against  \
                                this clock */
#define DESIGN_NUM 0u        /* mux slot; ignored by the ice40 ASIC-sim */
#define WAIT_FOR_USB 1
#define MAX_OPS 1024u /* ASIC PC is 10 bits */
#define PC_TIMEOUT_US 200000u
#define SERIAL_TIMEOUT_US 200000u

#define ASIC_CLKS_US(n) (((n) * 1000000u + ASIC_CLK_HZ - 1) / ASIC_CLK_HZ)
#define RX_SETTLE_US ASIC_CLKS_US(32u)  /* irq seen -> rx_enable ready */
#define SERIAL_HALF_US ASIC_CLKS_US(6u) /* MCU->ASIC serial half period; \
                                           serial_rx needs ~3 clks of    \
                                           synchronizer margin */
...
static void init_gpio(void) {
    static const uint out_pins[] = {
        PIN_INSTR0,      PIN_INSTR1,       PIN_INSTR2,      PIN_INSTR_VALID,
        PIN_RX_CLK,      PIN_RX_BIT,       PIN_SEL0,        PIN_SEL1,
        PIN_SPI_MISO,    TT_PIN_PROJ_NRST, TT_PIN_CTRL_ENA, TT_PIN_CTRL_NRST,
        TT_PIN_CTRL_INC, TT_PIN_LED,
    };
    static const uint in_pins[] = {
        PIN_TX_BIT,           PIN_TX_CLK,           PIN_IRQ_IO,
        PIN_IRQ_JUMP,         PIN_SPI_CS,           PIN_SPI_MOSI,
        PIN_SPI_SCK,          TT_GPIO_UO_BASE + 4,  TT_GPIO_UO_BASE + 5,
        TT_GPIO_UO_BASE + 6,  TT_GPIO_UO_BASE + 7,  TT_GPIO_UIO_BASE + 4,
        TT_GPIO_UIO_BASE + 5, TT_GPIO_UIO_BASE + 6, TT_GPIO_UIO_BASE + 7,
    };
    for (uint i = 0; i < count_of(out_pins); i++) {
        gpio_init(out_pins[i]);
        gpio_put(out_pins[i], 0);
        gpio_set_dir(out_pins[i], GPIO_OUT);
    }
    for (uint i = 0; i < count_of(in_pins); i++) {
        gpio_init(in_pins[i]);
        gpio_set_dir(in_pins[i], GPIO_IN);
    }
    /* CS idles high; a pull-up keeps core 1 quiet if the pin ever floats. */
    gpio_pull_up(PIN_SPI_CS);
}

static void start_project_clock(uint32_t hz) {
    gpio_set_function(TT_PIN_PROJ_CLK, GPIO_FUNC_PWM);
    gpio_set_input_enabled(TT_PIN_PROJ_CLK, true); /* we read it back to
                                                      sync instr_valid */
    uint slice = pwm_gpio_to_slice_num(TT_PIN_PROJ_CLK);
    uint32_t wrap = clock_get_hz(clk_sys) / hz;
    pwm_set_wrap(slice, wrap - 1);
    pwm_set_gpio_level(TT_PIN_PROJ_CLK, wrap / 2);
    pwm_set_enabled(slice, true);
}

/* Mux sequence per tt-micropython-firmware project_mux.py. Harmless on the
 * FPGA ASIC-sim carrier, required on real silicon. */
...
static bool read_program(void) {
    ...
    putchar('\n');
    if (overflow) {
        printf("!! program exceeds %u ops (ASIC PC is 10 bits) — "
               "discarded\n",
               MAX_OPS);
        return false;
    }
...
int main(void) {
    ...
    printf("ASIC clock %u Hz, design #%u\n", ASIC_CLK_HZ, DESIGN_NUM);

    start_project_clock(ASIC_CLK_HZ);
